...
def executer_algorithm_d_ordonnancement_enseignant(data, id_enseignant):
    ...
    for cours in donnees_horaire:
        ...
        # Vérifiez si le cours respecte les contraintes de limite d'enseignement
        if not verifier_limite_enseignement(id_enseignant, emploi_du_temps_par_jour[jour_semaine], donnees_contraintes):
            continue
...
# Fonction pour vérifier la limite d'enseignement pour un enseignant spécifique
def verifier_limite_enseignement(id_enseignant, emplois_du_temps_jour, donnees_contraintes):
    ...
    if contrainte is not None:
        ...
        # Calculez le nombre total d'heures d'enseignement pour l'enseignant spécifié
        heures_enseignement = sum(
            [convert_to_float(cours[0]) - convert_to_float(cours[1]) for cours in emplois_du_temps_jour])
